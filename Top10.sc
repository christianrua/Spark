...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val names = sc.textFile("../SparkScalaCourse/marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)
    
    // Load up the superhero co-apperarance data
    val lines = sc.textFile("../SparkScalaCourse/marvel-graph.txt")
    ...
    val mostPopular = flipped.take(10)
    
    //we get the top 10 of superheros
    //val mostPopular = flipped.topByKey(10)(Ordering[Int].reverse)
    
    // Look up the name (lookup returns an array of results, so we need to access the first result with (0)).
    
    for (popular <- mostPopular) {
      val mostPopularName = namesRdd.lookup(popular._2)(0)
      println(s"$mostPopularName is the most popular superhero with ${popular._1} co-appearances.")
    }
